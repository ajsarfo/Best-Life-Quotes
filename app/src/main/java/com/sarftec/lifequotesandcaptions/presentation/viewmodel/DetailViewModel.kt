...
import com.sarftec.lifequotesandcaptions.presentation.panel.AlignmentManager
import com.sarftec.lifequotesandcaptions.presentation.panel.BackgroundManager
        ...
        viewModelScope.launch {
            val quoteList = when (parcel.origin) {
                QuoteToDetail.ORIGIN_QUOTE -> {
                    repository.getQuotes(parcel.categoryId).shuffled(Random(parcel.seed))
                }
                QuoteToDetail.ORIGIN_MAIN -> {
                    listOf(repository.todayQuote())
                }
